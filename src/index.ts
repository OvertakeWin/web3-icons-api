...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    const png = url.searchParams.get("png") === "true";
    const ONE_YEAR = 31536000;
    const ONE_DAY = 86400;

    if (!chainId || (!address && !symbol)) {
      return new Response("Missing chainId and address or symbol", {
        status: 400,
      });
    }
    ...
    async function svgToCachedPngUrl(svg: string, size = 128) {
      ...
      const response = new Response(pngBuffer as Uint8Array<ArrayBuffer>, {
        headers: {
    ...
    const fetchWithCache = async (
      ...
      const upstreamRequest = new Request(
        bust ? `${upstreamUrl}?bust=${Date.now()}` : upstreamUrl,
      );
      ...
      const response = new Response(
        contentType === "image/svg+xml" ? await res.text() : res.body,
        {
          headers: {
    ...
    if (isNativeToken) {
      ...
      const alexandriaRes = await fetchWithCache(
        alexandriaChainUrl,
        "image/svg+xml",
      );
      if (alexandriaRes)
        return png
          ? svgToCachedPngUrl(await alexandriaRes.text())
          : alexandriaRes;
    ...
    if (address) {
      ...
      if (svgRes)
        return png ? svgToCachedPngUrl(await svgRes.text()) : svgRes;
    ...
    if (symbol) {
      ...
      const symbolRes = await fetchWithCache(
        web3iconsSymbolUrl,
        "image/svg+xml",
      );
      if (symbolRes)
        return png ? svgToCachedPngUrl(await symbolRes.text()) : symbolRes;
    }
